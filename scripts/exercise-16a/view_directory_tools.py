"""
Directory viewing tools for LangGraph ReAct / LCEL demonstrations.
...
class DirectoryListingTool(BaseTool):
    ...
    def _run(
        ...
        # Normalize the path
        try:
            normalized_directory = os.path.abspath(
                os.path.expanduser(cleaned_directory)
            )
        ...
        if not os.path.isdir(normalized_directory):
            return (
                f"[Error]: Path exists but is not a directory: {normalized_directory}"
            )
        ...
        try:
            ...
            for item in os.listdir(normalized_directory):
                if len(files) >= max_files:
                    files.append(
                        f"... (more files exist but limited to {max_files} for display)"
                    )
                    break
...
class DirectoryStructureTool(BaseTool):
    ...
    description: str = (
        "Shows a tree-like structure of directories and files (limited depth for readability)"
    )
    args_schema: Type[DirectoryListingInput] = DirectoryListingInput
                ...
                for i, item in enumerate(items):
                    ...
                    if os.path.isdir(item_path):
                        ...
                        try:
                            file_size = os.path.getsize(item_path)
                            tree_lines.append(
                                f"{current_prefix}{item} ({file_size} bytes)"
                            )
                        except Exception:
                            tree_lines.append(f"{current_prefix}{item}")
