"""
File viewing tools for LangGraph ReAct / LCEL demonstrations.
...
class ViewFileTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            with open(
                normalized_filepath, "r", encoding="utf-8", errors="replace"
            ) as file:
                content = file.read()
...
class ViewFileLinesTool(BaseTool):
    ...
    def _run(
        ...
        if end_line < start_line:
            return (
                f"[Error]: End line ({end_line}) must be >= start line ({start_line})"
            )
